...
void GameofLife::simulate(){
  ...
  for(int i=0;i < board->height; i++){
    for(int j=0;j < board->length; j++){
      ...
      n = board->getCell(i-1,j-1) + board->getCell(i-1,j) + board->getCell(i-1,j+1) + board->getCell(i,j-1) + board->getCell(i,j+1) + board->getCell(i+1,j-1) + board->getCell(i+1,j) + board->getCell(i+1,j+1);
